...
export class FrontmatterProcessor {
     ...
     */
    static parseSettings(markdown: string): Partial<MarpSettings> {
        try {
            const { frontmatter, hasManualFrontmatter } = this.extractFrontmatter(markdown);
     ...
     */
    static updateFrontmatter(markdown: string, newSettings: Partial<MarpSettings>): string {
        const { content } = this.extractFrontmatter(markdown);

        let currentSettings = this.parseSettings(markdown);
     ...
     */
    static mergeContentWithFrontmatter(originalMarkdown: string, newContent: string): string {
        const { frontmatter, hasManualFrontmatter } = this.extractFrontmatter(originalMarkdown);
